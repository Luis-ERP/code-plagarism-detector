...
class Runner:
    def __init__(self, mode: str, version: int, tool: str) -> None:
        ...
        if tool == 'custom':
            self.algorithm = CustomAlgorithm(self.version, quiet=True)
        # if tool == 'moss':
...
if __name__ == '__main__':
    ...
    tool_help = 'tool name to run: sherlock, sim, moss, dolos, jplag, bplag, custom'
    ...
    if args.tool is None:
        ...
        tool = args.tool[0]
        if tool not in ['sherlock', 'sim', 'moss', 'dolos', 'jplag', 'bplag', 'custom']:
            process_error(parser)
